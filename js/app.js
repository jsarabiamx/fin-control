...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    manejarError(err, 'No se pudo cargar la informacion desde Supabase.');
  }
});
...
async function agregarGasto() {
  ...
  try {
    await DB.addGasto({ tipo:'gasto', concepto, monto, categoria, fecha });
    clearGastoForm();
    cargarCategorias('g-categoria');
    actualizarUI();
  } catch (err) {
    manejarError(err, 'No se pudo guardar el gasto.');
  }
}
...
async function guardarEdicionGasto() {
  ...
  try {
    await DB.editGasto(editGastoId, {
    ...
    manejarError(err, 'No se pudo actualizar el gasto.');
  }
}
...
function pedirEliminarGasto(id, concepto) {
  mostrarConfirmar(
    ...
    async () => {
      try {
        await DB.delGasto(id);
        cerrarModal('modal-confirmar');
        actualizarUI();
      } catch (err) {
        manejarError(err, 'No se pudo eliminar el gasto.');
      }
    }
  );
}
...
async function agregarIngreso() {
  ...
  try {
    await DB.addIngreso({ tipo:'ingreso', concepto, monto, fecha });
    cerrarModal('modal-ingreso');
    actualizarUI();
  } catch (err) {
    manejarError(err, 'No se pudo guardar el ingreso.');
  }
}
...
async function guardarEdicionIngreso() {
  ...
  if (isNaN(monto) || monto <= 0) return;
  try {
    await DB.editIngreso(editIngresoId, {
    ...
    manejarError(err, 'No se pudo actualizar el ingreso.');
  }
}
  ...
  mostrarConfirmar(
    ...
    () => {
      ...
      mostrarConfirmar(
        ...
        async () => {
          try {
            await DB.delIngreso(id);
            cerrarModal('modal-confirmar');
            actualizarUI();
          } catch (err) {
            manejarError(err, 'No se pudo eliminar el ingreso.');
          }
        }
      );
    }
...
async function guardarPresupuesto() {
  ...
  try {
    await DB.setPresupuesto(v);
    cerrarModal('modal-presupuesto');
    actualizarUI();
  } catch (err) {
    manejarError(err, 'No se pudo actualizar el presupuesto.');
  }
}
...
function manejarError(err, fallbackMsg) {
  console.error(err);
